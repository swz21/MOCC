...
class Network():
    ...
    def __init__(self, senders, links, weight_thr, weight_lat, weight_loss):
        self.q = []
        self.cur_time = 0.0
    ...
    def run_for_dur(self, dur):
        ...
        reward = self.weight_thr * throughput / 8 / BYTES_PER_PACKET / self.links[0].bw + self.weight_lat * 2 * self.links[0].dl / max(latency, 1e-5) + self.weight_loss * (1 - loss)
        ...
        #if reward > 857:
        # print("Reward = %f, thpt = %f, lat = %f, loss = %f" % (reward, throughput / 8 / BYTES_PER_PACKET / self.links[0].bw, 2 * self.links[0].dl / max(latency, 1e-5), (1 - loss)))
...
class SimulatedNetworkEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.min_queue, self.max_queue = (0, 4)
        self.min_loss, self.max_loss = (0.0, 0.03)
        self.history_len = history_len
        print("History length: %d" % history_len)
        self.features = features.split(",")
        print("Features: %s" % str(self.features))

        self.weight_thr = weights[0]
        self.weight_lat = weights[1]
        self.weight_loss = weights[2]
        print("Weights: %s" % str(weights))

        self.links = None
        self.senders = None
        self.create_new_links_and_senders()
        self.net = Network(self.senders, self.links, self.weight_thr, self.weight_lat, self.weight_loss)
        self.run_dur = None
        self.run_period = 0.1
        self.steps_taken = 0
        ...
        self.observation_space = spaces.Box(np.append(np.tile(single_obs_min_vec, self.history_len), [0.0, 0.0, 0.0]),
                                            np.append(np.tile(single_obs_max_vec, self.history_len), [1.0, 1.0, 1.0]),
                                            dtype=np.float32)
    ...
    def step(self, actions):
        ...
        for sender in self.senders:
            sender.record_run()
        self.steps_taken += 1
        sender_obs = np.append(self._get_all_sender_obs(), [self.weight_thr, self.weight_lat, self.weight_loss])
        sender_mi = self.senders[0].get_run_data()
        event = {}
        event["Name"] = "Step"
    ...
    def reset(self):
        ...
        if self.episodes_run > 0 and self.episodes_run % 100 == 0:
            # os.makedirs(f"./data_{self.weight_thr}_{self.weight_lat}_{self.weight_loss}", exist_ok=True)
            os.makedirs(f"./data_ft", exist_ok=True)
            self.dump_events_to_file(f"./data_{self.weight_thr}_{self.weight_lat}_{self.weight_loss}/pcc_env_log_run_%d.json" % self.episodes_run)
        self.event_record = {"Events":[]}
        ...
        self.reward_sum = 0.0
        return np.append(self._get_all_sender_obs(), [self.weight_thr, self.weight_lat, self.weight_loss])
